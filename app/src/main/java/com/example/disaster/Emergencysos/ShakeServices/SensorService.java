package com.example.disaster.Emergencysos.ShakeServices;
                        ...
                        public void onSuccess(Location location) {
                            ...
                            if(location!=null){
                               ...
                               SmsManager smsManager = SmsManager.getDefault();
                                Log.d("SensorService", "pooja");

                                //get the list of all the contacts in Database
                                DbHelper db=new DbHelper(SensorService.this);
                                List<ContactModel> list=db.getAllContacts();
                                Log.d("ContactList", list.toString());
                                //send SMS to each contact
                                for(ContactModel c: list){
                                    Log.d("SensorService", "sending sms");
                                    ...
                                    Log.d("SensorService", "sms sent");

                                }
                            }else{
                                String message= "I am in DANGER, i need help. Please urgently reach me out.\n"+"GPS was turned off.Couldn't find location. Call your nearest Police Station.";
                                SmsManager smsManager = SmsManager.getDefault();
                                Log.d("SensorService", "sent sms");

                                DbHelper db=new DbHelper(SensorService.this);
                                List<ContactModel> list=db.getAllContacts();
                                for(ContactModel c: list){
                                    Log.d("SensorService", "fetching cotact");

                                    smsManager.sendTextMessage(c.getPhoneNo(), null, message, null, null);
                                    Log.d("SensorService", "contact fetched");
